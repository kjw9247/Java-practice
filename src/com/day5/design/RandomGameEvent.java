...
// RandomGameEvent는 ActionListener의 구현체 클래스가 되어야 한다
public class RandomGameEvent implements ActionListener {
    ...
    public RandomGameEvent(RandomGameUI rgu){
        // 직접 인스턴스화 하는게 아니고 새성자 파라미터를  통해서
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (obj == rgu.btnDap){
            ...
            // 변수 dap은 nanSu() 메소드에서 결정된다
            // 변수 my는 언제 재정의 되나요? - 사용자가 입력한 값 - 화면
            // JTextField에 작성된 값
            // my는 사용자가 입력할 때 마다 변경되고
            // 또 my는 dap변수와 비교해야 한다
            // 클래스 분리한 상황에서도 my를 전변으로 해야 하나여?
            // 아니면 지변으로 해도 되는가?
            // my는 jtf에 엔터를 쳤을 때 결정된다
