...
public class JDBCDemo {
    private final String _DRIVER = "oracle.jdbc.driver.OracleDriver"; // 제조사
    ...
    // 동적쿼리는 변수자리에 물픔표를 사용한다
    // 물음표 자리가 다른 것은 같은 쿼리로 인지한다 - 오라클이
    // 파싱과 실행계획을 생략하고 3단계부터 처리한다
    PreparedStatement pstmt = null; // 동적쿼리
    ResultSet rs = null; // 커서를 조작한다
    // commit과 rollback 대상이다
    // 사원등록 - insert - rest api post
    // ( post 와 get의 파라미터 처리방법은 다름- 단위테스트 방법이 달라짐 )
    // INSERT INTO emp(empno, ename, job,,,)
    // VALUES('800', '강감찬', '세일즈',,,,
    // 오라클의 반환값 - 1 row inserted
    ...
    // DELETE FROM emo WHERE empno = 8000
    // 메서드 설계시 파라미터를 쿼리문을 보고 결정할 수 있다
    public int empDelete(int deptno){
        int result = 0;
        StringBuilder query = new StringBuilder();
        query.append("DELETE FROM emo WHERE deptno = ?");
        try{
            Class.forName(_DRIVER);
            conn = DriverManager.getConnection(_URL, _USER, _PASSWORD);
            pstmt = conn.prepareStatement(query.toString());
    ...
    // 사원조회 - select
    // 하나의 메소드에서 전체 조회일 경우와 단 건 조회일 경우를 같이 쓴다
    // if문
    public List<EmpVO> empList (EmpVO peVO){
        ...
        try{
            ...
            // 아래에 while문을 작성하지 않아도 MyBatis가 자료구조안에
            // 자동으로 담아 준다
            while(rs.next()){
                EmpVO evo = null;
...
n건 - List, Object[]
insert, update, delete : int가 변환타입이다

insert는 8개 칼럼을 사용자로 부터 입력 받는다
파라미터는 8개가 필요하다
만일 8개를 한번에 넘기고 한번에 받아오는 방법은 ?
...
delete는 기본에 있는 정부 중 한 개 로우를 삭제하는 것이다
조건절에는 사원번호가 있다.
...
테이블에 변화가 없다
commit과 rollback의 대싱이 아니다

insert, update, delete 커서와는 무관하다
ResultSet은 필요가 없다
select는 조회 결과가 있어서 ResultSet 필요하다

insert, update, delete - 리턴값이 정수다
1 row inserted, 1 row updated, 1 row deleted
